...
// 获取用户id
const USER_ID_URL = 'https://qyapi.weixin.qq.com/cgi-bin/auth/getuserinfo';
// 获取用户完整信息
...
module.exports = class OAuth {
  ...
  getAuthorizeUrl(options) {
    ...
    return AUTHORIZE_URL + '?' + querystring.stringify(params)+'#wechat_redirect';
  }
  ...
  async getUserInfo(accessToken, code) {
    ...
    const { userid,user_ticket } = idInfo;

    const infoUrl = USER_INFO_URL + '?' + querystring.stringify({ access_token: params.access_token, userid });
    let userInfo = await wechatRequest(infoUrl)
    userInfo['user_ticket'] = user_ticket
    return userInfo
  }
};
